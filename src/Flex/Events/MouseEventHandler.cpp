...
void MouseEventHandler::handleEvent(const sf::Event& event) {
    event.visit(this);
}

void MouseEventHandler::operator()(const sf::Event::MouseButtonPressed& event) {
...
void MouseEventHandler::operator()(const sf::Event::MouseButtonReleased& event) {
    int button = static_cast<int>(event.button);
    if (button < 0 || button >= MAX_BUTTONS) {
        return;
    }
